...
const Schema = v.pipe(
  v.object({
    ...
    startDate: vRequiredDate('시작일시을 선택해주세요.'),
    endDate: vRequiredDate('종료일시을 선택해주세요.'),
  }),
  vDateRange({
    startKey: 'startDate',
    endKey: 'endDate',
...
export default function DateTimeFormUI() {
  ...
  return (
    <AppForm form={form}>
      <form.AppField name="requiredDate">
        {(field) => <field.DateTimePicker label="Required Date" isRequired />}
      </form.AppField>

      <form.AppField name="minDate">
        {(field) => <field.DateTimePicker label="Min Date" />}
      </form.AppField>

      <form.AppField name="requiredMinDate">
        {(field) => (
          <field.DateTimePicker label="Required Min Date" isRequired />
        )}
      </form.AppField>

      <form.AppField name="maxDate">
        {(field) => <field.DateTimePicker label="Max Date" />}
      </form.AppField>

      <form.AppField name="requiredMaxDate">
        {(field) => (
          <field.DateTimePicker label="Required Max Date" isRequired />
        )}
      </form.AppField>

      <div className="flex gap-sw-sm">
        <form.AppField name="startDate">
          {(field) => <field.DateTimePicker label="Start Date" isRequired />}
        </form.AppField>
        <form.AppField name="endDate">
          {(field) => <field.DateTimePicker label="End Date" isRequired />}
        </form.AppField>
      </div>
